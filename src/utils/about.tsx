import JeffImg from '../../public/media/Jeff.jpg'
import DylanImg from '../../public/media/Dylan.jpg'
import FranImg from '../../public/media/Fran.jpg'
import DinaImg from '../../public/media/Dina.jpg'

export const teamMembers = [
    {
        img: JeffImg.src,
        summary:
            'Jeff, an expert in aging in place, founded 360 Degree Care in 2014. Pioneering Concierge Level™ Homecare with compassionate, presonalized support for aging individuals and families.',
        expanded:
            'Jeff is an acknowledged expert in the growing field of aging in place. He has owned and operated 360 Degree Care since its founding in 2014. In that time we have written articles and conducted events that focus on the question of the sandwich generation, the physical changes of aging and developing original music programs for Alzheimer’s and dementia patients. Jeff created the concept of Concierge Level™ Homecare, bringing his vast experience servicing high-end, complex client situations in the advertising industry where he served in senior management positions at Chiat/Day, Cates Worldwide, J. Walter Thompson and Omnicom handling clients such as Goldman Sachs, Honeywell, Unilever, Drexel and more. ',
        name: 'Jeff DeJoseph',
        position: 'Chairman & CEO',
        email: 'jeff@360degreecare.net'
    },
    {
        img: DylanImg.src,
        summary:
            'Dylan manages all client billing, records, recruitment, and retention at 360, achieving an exceptional 80% caregiver retention rate compared to the industry average of 13%, and is also an accomplished guitarist who plays with the band Metalfier in NYC venues.',
        expanded:
            'Dylan is responsible for all client billing and records as well as recruitment and retention. This has been particularly successful as 360 maintains an 80% retention rate of our caregivers—unheard of in the homecare industry where retention has hovered around 13% for decades.  Outside of 360, Dylan is an accomplished guitar and bass player often sitting in with the band Metalfier at various clubs in NYC and Brooklyn.',
        name: 'Dylan James',
        position: 'Account Coordinator',
        email: 'dylan@360degreecare.net'
    },
    {
        img: FranImg.src,
        summary:
            'Fran, a College of New Rochelle nursing graduate with experience at Stamford Hospital and Caring People, joined 360 at its founding in 2014 after raising three children and volunteering in Ridgewood, and is now an integral team member with five grandchildren.',
        expanded:
            'Fran began her nursing career after graduation from the College of New Rochelle. She spent 4 years at Stamford Hospital on the surgical floor. Leaving to raise her 3 children who are now raising 5 grandchildren, she became an important member of the Ridgewood volunteer community. Resuming her nursing at Caring People, a homecare company, she joined 360 in 2014 at the company’s founding. She is an integral part of our company.',
        name: 'Fran Baker',
        position: 'SVP Director of Healthcare Services',
        email: 'fran@360degreecare.net'
    },
    {
        img: DinaImg.src,
        summary:
            'Dina leverages her advertising background to excel at strategically matching caregivers to clients at 360, bringing compassionate yet precise service to those with complex needs, while balancing her personal life in Bronxville with her husband, puppies, and two successful daughters.',
        expanded:
            'Beginning her career in the advertising business Dina learned how to service clients with complex needs and an extremely low tolerance for slip-ups. At 360 she excels at matching exactly the right caregiver to the client. She’s dedicated to improving the lives of our clients through personalized and professional care. Her keen intuition makes her a compassionate yet strategic force in both business and care. Dina lives in Bronxville Ny with her husband Jack and their puppies. She has raised 2n successful daughters through both strength and a delicate hand.',
        name: 'Dina Rose',
        position: 'SVP Administrator',
        email: 'dina@360degreecare.net'
    }
]
