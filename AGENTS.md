...
- Service and location experiences stitch together structured content from `src/lib/content/**`, SEO templates in `src/lib/seo/**`, and dynamic generators in `src/lib/locationSeo.ts` to deliver county, city, and service-specific metadata and copy.
...
- `src/app/` – App Router pages, including service hubs, `/location/[county]/[city]` dynamic routes, blog pages, documentation views, legal pages, and contact funnels.
...
- `src/lib/` – Data and infrastructure: county + city definitions, service registries, testimonial/content catalogs, analytics helpers, React context providers, and SEO component templates.
...
- `public/` – Static assets plus sitemap files generated by `next-sitemap` (`sitemap.xml`, `sitemap-0.xml`) and the web app manifest.
...
| `/services/[service]/[city]` | Dynamic (SSG) | Generates service pages per city by combining `generateLocationSEO` output with service template overrides. | `src/app/services/<service>/[city]/page.tsx` |
| `/location/[county]`, `/location/[county]/[city]` | Dynamic (SSG) | County hubs and city detail pages assembled from `src/lib/counties` data (hero, service listings, CTAs). | `src/app/location/[county]/page.tsx`, `src/app/location/[county]/[city]/page.tsx` |
...
- **Sitemaps & robots** – `next-sitemap.config.ts` uses county/service data to precompute XMLs after each build (`npm run build` automatically runs `next-sitemap`).
- **Static content** – Services, counties, testimonials, and blog entries live as TypeScript modules (`src/lib/content/**`, `src/lib/blogs/articles/**`), enabling static generation without external CMS calls.
...
- Keep service/location data centralized: update `src/lib/locationSeo.ts`, `src/lib/content/**`, and `src/lib/seo/serviceTemplates/**` together when introducing new geographies or offerings.
...
- `npm run dev` for manual verification; ensure dynamic routes (`/services/.../[city]`, `/location/...`) render without 404s.
...
- After production builds, confirm `public/sitemap.xml` and `public/sitemap-0.xml` regenerate and include any new routes.
